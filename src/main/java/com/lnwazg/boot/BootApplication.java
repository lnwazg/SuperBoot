...
public class BootApplication {
	...
	public static void run(Class<?> mainClazz, String[] args) {
		...
		// 获取jdbc对象
		MyJdbc jdbc = DbKit.getJdbc(appConfigs.get("db.url"), appConfigs.get("db.username"),
		...
		// 启动定时任务调度器
		if (StringUtils.isNotEmpty(appConfigs.get("mvc.packagesearch.job"))) {
			JobLoader.loadPackageJob(appConfigs.get("mvc.packagesearch.job"));
		}

		// 启动后立即要做的一系列事情
		if (StringUtils.isNoneEmpty(appConfigs.get("afterboot.tasklist.classpath"))) {
			HandlerSequence.executeTasksByTaskClazzPathAsync(appConfigs.get("afterboot.tasklist.classpath"));
		}
	}
}
